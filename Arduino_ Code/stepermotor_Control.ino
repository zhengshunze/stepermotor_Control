  ...
  if (newData == true) //we only enter this long switch-case statement if there is a new command from the computer
  {
    ...
    switch (opt) {
      ...
      case 'a':  //利用setAcceleration設定加速度值
        ...
        if (stepperNum[1] == 1) {
         // Serial.print(F("馬達2 'setAcceleration': "));
         // Serial.println(data[1]);
      ...
      case 'x':  //利用setMaxSpeed函数設定最大速度值
        ...
        if (stepperNum[1] == 1) {
         // Serial.print(F("馬達2 'setMaxSpeed' "));
          //Serial.println(data[1]);
          stepper2.setMaxSpeed(data[1]);
        }
        if (stepperNum[2] == 1) {
          //Serial.print(F("馬達3 'setMaxSpeed' "));
          //Serial.println(data[2]);
          stepper2.setMaxSpeed(data[2]);
        }
        break;
  case 'c':  //利用setMaxSpeed函数設定最大速度值
...
if (stepperNum[1] == 1) {
   ...
   break;
      ...
      case 'd':  //設定使哪個馬達做動
        ...
        else if (  !(data[0] + data[1] + data[2] == 0 || data[0] + data[1] + data[2] == 1 || data[0] + data[1] + data[2] == 2
